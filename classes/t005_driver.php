...
/**
 * Table class for t005_driver
 */
class t005_driver extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 't005_driver';
		$this->TableName = 't005_driver';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t005_driver`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('t005_driver', 't005_driver', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// Nama
		$this->Nama = new DbField('t005_driver', 't005_driver', 'x_Nama', 'Nama', '`Nama`', '`Nama`', 200, -1, FALSE, '`Nama`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// No_HP_1
		$this->No_HP_1 = new DbField('t005_driver', 't005_driver', 'x_No_HP_1', 'No_HP_1', '`No_HP_1`', '`No_HP_1`', 200, -1, FALSE, '`No_HP_1`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->No_HP_1->Nullable = FALSE; // NOT NULL field
		$this->No_HP_1->Sortable = TRUE; // Allow sort
		$this->fields['No_HP_1'] = &$this->No_HP_1;

		// No_HP_2
		$this->No_HP_2 = new DbField('t005_driver', 't005_driver', 'x_No_HP_2', 'No_HP_2', '`No_HP_2`', '`No_HP_2`', 200, -1, FALSE, '`No_HP_2`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->No_HP_2->Nullable = FALSE; // NOT NULL field
		$this->No_HP_2->Sortable = TRUE; // Allow sort
		$this->fields['No_HP_2'] = &$this->No_HP_2;
	}
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`t005_driver`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->No_HP_1->DbValue = $row['No_HP_1'];
		$this->No_HP_2->DbValue = $row['No_HP_2'];
	}
	...
	{
		global $Language;
		if ($pageName == "t005_driverview.php")
			return $Language->phrase("View");
		elseif ($pageName == "t005_driveredit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "t005_driveradd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("t005_driverview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t005_driverview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "t005_driveradd.php?" . $this->getUrlParm($parm);
		else
			$url = "t005_driveradd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("t005_driveredit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("t005_driveradd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("t005_driverdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->No_HP_1->setDbValue($rs->fields('No_HP_1'));
		$this->No_HP_2->setDbValue($rs->fields('No_HP_2'));
	}
	...
	public function renderEditRow()
	{
		...
		$this->No_HP_1->EditValue = $this->No_HP_1->CurrentValue;
		...
		$this->No_HP_2->EditValue = $this->No_HP_2->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->No_HP_2);
				} else {
					$doc->exportCaption($this->id);
					$doc->exportCaption($this->TruckingVendor_id);
					$doc->exportCaption($this->Nama);
